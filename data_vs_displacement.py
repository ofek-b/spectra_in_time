...
def twospec_vs_displacement(snname, fixed_mjd, friends_type):
    ...
    friends = [i for i, sn in enumerate(snlist_) if sn.type == friends_type]
    ...
    lst = sorted(zip(times_2nd_specs, comparison), key=lambda tup: tup[0])
    lst = [tup for tup in lst if tup[0] < max(TIME)]
    times_2nd_specs, comparison = zip(*lst)
    plt.plot(times_2nd_specs, comparison, 's-', color='k')
    ax = plt.gca()
    plt.axvline(maxtime, linestyle='--', color='grey', label=' Fixed first spectrum at B-band maximum')
    # plt.axvline(maxtime, linestyle='--', color='green', label='Fixed First Spectrum')
    # ax.annotate('B-max', xy=(maxtime, 0.85), xycoords=('data', 'axes fraction'))
    plt.axhline(orig_mean_to_diam, linestyle='--', color='blue', label='Using full data')
    ...
    plt.ylabel('Mean distance to cluster / cluster diameter')
    plt.xlabel('Epoch of the second spectrum [days since explosion]')
    plt.grid()
    plt.show()
...
if __name__ == '__main__':
    ...
    twospec_vs_displacement('SN2005cf', 53533, 'Ia')
    twospec_vs_displacement('SN2013fs', 56575, 'II')
